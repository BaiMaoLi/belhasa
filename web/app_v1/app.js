		...
		function($stateProvider, $urlRouterProvider) {
			$stateProvider
				...
				.state('app', {
					...
					onEnter: function(Authentication, $state) {
						if (!Authentication.user) {
							console.log("Not Authenticated in App Route");
							$state.go('login');
						}
					}
				})
				...
				.state('app.question_bulk_import', {
					url: '/questions/bulk_import?category&type',
					views: {
						'section@app': {
							templateUrl: 'app_v1/templates/question_bulk_import.tpl.html',
							controller: 'QuestionController'
						}
					},
					onEnter: function(Authentication, $state) {
				...
				// .state('otherwise', { url : '/loading'})
				// .state('otherwise', { url : '/loading'})
				// .state('otherwise', { redirectTo: '/loading' })
...
var $app = angular
	...
	$app.run(function($rootScope) {
		$rootScope.getQuestionImage = function(image) {
			return image.length>100 ? image : 'app/assets/images/question photos/'+image;
		};
	});
